...
for i in $(seq 1 $ITERATIONS); do
    echo -n "."
    time=$(curl -s -w "%{time_total}" -o /dev/null $HOST/health)
    health_times+=($time)
done
echo ""
avg_health=$(calculate_average "${health_times[@]}")
echo "Average response time for health endpoint: ${avg_health}s"

# Test get all users endpoint
echo "Testing get all users endpoint"
users_times=()
for i in $(seq 1 $ITERATIONS); do
    echo -n "."
    time=$(curl -s -w "%{time_total}" -o /dev/null $HOST/users)
    users_times+=($time)
done
echo ""
avg_users=$(calculate_average "${users_times[@]}")
echo "Average response time for get all users endpoint: ${avg_users}s"

# Create a user for GET by ID tests
echo "Creating a test user"
USER_ID=$(curl -s -X POST \
  -H "Content-Type: application/json" \
  -d '{"name":"Benchmark User","email":"benchmark@example.com","age":42}' \
  $HOST/users | jq -r '.id')
...
for i in $(seq 1 $ITERATIONS); do
    echo -n "."
    time=$(curl -s -w "%{time_total}" -o /dev/null $HOST/users/$USER_ID)
    user_id_times+=($time)
done
echo ""
avg_user_id=$(calculate_average "${user_id_times[@]}")
echo "Average response time for get user by ID endpoint: ${avg_user_id}s"
